export default {
  ...
  confirmations: {
    ...
    numberConfirmation: 'После подтверждения средства будут списываться автоматически c привязанной карты. Вы уверены, что хотите продолжить?',
    cardApplied: 'Ваша банковская карта успешно привязана.',
    cardPayments: 'Списание средств будет происходить автоматически после одобрения договора.',
  ...
  statuses: {
    approved: {
      title: 'Кридит оформлен',
      description: 'Вы можете ознакомиться с официальным документом или скачать его для дальнейшего использования.',
    },
    rejected: {
      title: 'Отклонено',
      description: 'К сожалению, рассрочка не была одобрена. Вы сможете повторно подать заявку через 3 месяца.',
    },
    close: {
